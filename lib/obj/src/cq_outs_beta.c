...
signed(__cdecl cq_outs_beta(KNOT(*k),CMDLN_STAT(*argp))) {
...
/* Occur a coordinate error
r = sustain(argp);
if(!r) {
printf("<< Error at fn. sustain()");
return(0x00);
}
//*/
...
r = cq_outs_beta_internal(r,k,argp);

if(!r) {
printf("%s", "<< Error at fn. cq_outs_beta_internal()");
return(0x00);
}
