...
Use at fn. cmdln_ctrl_i

Remarks:
Launch on vu.exe
Refer at incl/cmdln.h and incl/config.h for the CMDLN_STAT structure
and at fn. cmdln_ctrl_i.c
*/
...
signed(__cdecl qclear_i(CMDLN_STAT(*argp))) {
...
l = (-coord_b.Y+((*argp).csbi.srWindow.Bottom));
INC(l);
l = (l*(0x01+((*argp).csbi.srWindow.Right)));

r = FillConsoleOutputCharacter((*argp).s_out,ws,l,coord_b,&i);
