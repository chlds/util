...
signed(__cdecl cmdl_time_b_r(sat_t(*argp))) {
...
if(!(CLI_ALREADY&(cli_flag))) OR(*(CLI_BASE+(R(flag,*argp))),CLI_ALREADY);
else {
cli_flag = (CLI_ALREADY);
NOT(cli_flag);
AND(*(CLI_BASE+(R(flag,*argp))),cli_flag);
return(0x01);
}
