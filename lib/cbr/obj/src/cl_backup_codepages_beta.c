...
signed(__cdecl cl_backup_codepages_beta(CLI_STAT(*argp))) {
...
*(CLI_INDEX+R(io,R(codepage,*argp))) = GetConsoleCP();
if(!(*(CLI_INDEX+R(io,R(codepage,*argp))))) {
r = GetLastError();
printf("%s%d%s%X\n","<< Error at fn. GetConsoleCP() with error no. ",r," or ",r);
return(0x00);
}

*(CLI_LEAD+R(io,R(codepage,*argp))) = GetConsoleOutputCP();
if(!(*(CLI_LEAD+R(io,R(codepage,*argp))))) {
r = GetLastError();
